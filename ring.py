...
#
# s1d_mean_all = np.log(s1d_mean_all)
...
if args.verbose :
    ...
    #pbar = tqdm.tqdm(total=height, update_to=True)
if False :
#for i in tqdm.tqdm(range(height), disable=not args.verbose):

    sino_org = np.array(inData[:,i,:])
    sino_org = np.where(sino_org <= 0, 1e-6, sino_org)
    sino_org = -np.log(sino_org)
    ...
    # store
    sino = np.clip(sino, -38, 38)
    sino = np.exp(-sino)
    outWrapper.put(sino, np.s_[:,i,:], flush=False)


lockPool = Lock()
def cleanRingInPool(i) :
    global inData, s1d_mean_all, args, outWrapper, pbar, lockPool
    sino_org = np.array(inData[:,i,:])
    sino_org = np.where(sino_org <= 0, 1e-6, sino_org)
    sino_org = -np.log(sino_org)
    sino = np.copy(sino_org)
...
dataInPool = [ i for i in range(height) ]
with Pool() as p, tqdm.tqdm(total=height, disable=not args.verbose) as pbar: # CPU load
    for result in p.imap(cleanRingInPool, range(height)):
        outWrapper.put(result[0], np.s_[:,result[1],:], flush=False)
        pbar.update()
        pbar.refresh()
